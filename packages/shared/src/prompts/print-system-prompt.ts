#!/usr/bin/env bun
/**
 * Debug script to print the full WS Workspace system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    WS WORKSPACE SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. WS Workspace Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.ws-workspace/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@ws-workspace/electron/main.log' },
  '/Users/example/.ws-workspace/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.ws-workspace/workspaces/abc123/sessions/260121-example-session/plans',
});
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// WS Workspace additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. WS Workspace Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
