...
`@craft-agent/shared` is the core business logic package for WS Workspace. It contains:
...
- Workspace: `~/.ws-workspace/workspaces/{id}/permissions.json`
- Source: `~/.ws-workspace/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.ws-workspace/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.ws-workspace/theme.json`
- Workspace: `~/.ws-workspace/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.ws-workspace/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.
...
│     └── ~/.ws-workspace/workspaces/{ws}/sources/{slug}/          │
└─────────────────────────────────────────────────────────────────┘
                              │
...
Multi-workspace configuration stored in `~/.ws-workspace/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.ws-workspace/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.
